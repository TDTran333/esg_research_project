...
# -------------------------------------------------------------------------
.f_plot_mean_pi <- function(df, input_date, name){
  {{ df }} %>% 
    f_summ_pi({{ input_date }}) %>% 
    as_tibble() %>% 
    mutate(model = {{ name }}) %>% 
...
f_plot_mean_pi <- compiler::cmpfun(.f_plot_mean_pi)

# -------------------------------------------------------------------------
.f_plot_hist_obs <- function(df, input_date, name) {
  
  {{ df }} %>% 
    bind_rows() %>% 
    mutate(date = as.Date.yearqtr({{ input_date }})) %>% 
    pivot_longer(-date, names_to = "permno", values_to = "obs") %>%
    filter(!is.na(obs)) %>%
    ggplot(aes(obs)) +
    geom_histogram(bins = 30) +
    facet_wrap(~date) +
    labs(title = paste0("Concordant observations for ", {{ name }}))
} 
